...
public class RegisterServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            tblUser.setFullName(userid);
            tblUser.setPassword(password);
            UserDAO.getInstance().create(tblUser);
            request.getRequestDispatcher("LoginServlet?userId=" + userid + "&password=" + password).forward(request, response);
            //response.sendRedirect("LoginServlet?userId=" + userid + "&password=" + password);
        } catch (Exception e) {
        }
    }
